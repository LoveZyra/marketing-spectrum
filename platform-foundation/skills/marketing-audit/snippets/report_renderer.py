...
class ReportRenderer:
    ...
    def _humanize_feature(name: str) -> str:
        """字段名 → 中文标签。2026-08-14 起委托 feature_labels(圈人出参 sql_to_zh
        与报告共用同一份表,两处叫法必须一致);import 不到时按平铺布局兜底。"""
        try:
            from .feature_labels import feature_label
        except Exception:  # noqa: BLE001 —— 平铺引用时的兜底
            try:
                from feature_labels import feature_label
...
def _appendix(rr, segments, seg_back=None) -> str:
    ...
    for k, s in enumerate(segments):
        ...
        # fix30:优先对 filter_conditions_sql 做 sql_to_zh 逐 token 直译 —— 与 API 出参
        # filter_zh 完全同一句话(报告和接口两处叫法一致),且 pandas 形态里的 __NA__
        # 双表示守卫不再漏给读者。翻不动/没有 SQL 再退回 humanize(pandas)。
        ...
        if _sqlc:
            try:
                try:
                    from .crowd_translator import sql_to_zh as _s2z
                except ImportError:
                    from crowd_translator import sql_to_zh as _s2z
                _zh_cond = _s2z(_sqlc) or ""
            except Exception:                    # noqa: BLE001 —— 展示层永不崩
                _zh_cond = ""
        if not _zh_cond:
            _zh_cond = humanize_condition(_pd_cond)
