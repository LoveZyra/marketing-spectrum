...
## ★ 2026-08-12 人群规则出参新增中文口径 filter_zh(fix29,需配套 API ma-fix19)

**动机**:fix28 让报告里的筛选条件能读懂了,但**接口出参仍然只有英文** —— `crowd_rules.json` 里是 `pandas_filter` + `sql_filter`,`/result` 对外给的是 `sql_filter`。运营在报告里看得懂、在接口里看不懂,同一个口径两副面孔。

**做法**:`crowd_translator.build_crowd_rules()` 每条规则新增 `filter_zh`,与报告附录「筛选条件（中文）」**同一套翻译**(有断言逐条比对两处必须一字不差)。

**三条约束**:

1. **纯附加** —— 原有键一个不少、值一个字不改,`sql_filter` 仍是唯一执行口径。回归里逐条比对 `pandas_filter` 与原文;
2. **翻不动交空串** —— `humanize_condition` 翻不出时会**原样回显**输入,直接用等于把英文伪装成中文口径发出去。判据是 `译文 != 原文`;
3. **翻译炸了不能拖垮圈人链路** —— `report_renderer` 是**延迟导入**的,import 失败或翻译抛异常都只让这一个字段退化为空串,其余照常产出(有用例:把 `humanize_condition` 换成必抛的桩,规则条数与 `sql_filter` 逐条不变)。

**⚠️ API 侧必须同步升级**:`/result` 是**白名单投影**(`ma_core.py`,逐条只给 name/finding_id/sql_filter/direction/suggestion),skill 这边加多少字段都到不了调用方,只会躺在 `meta.json` 里。配套包 `ma-fix19` 把投影加到六个字段,并把这段逻辑抽成 `public_rules()` + 新增 `regress_contract.py`(22 项)守住契约 —— 此前这段代码埋在 handler 里,加字段全靠人眼盯。

**连带修掉两个翻译缺陷**(都是拿真实规则试出来的,不是预防性改动):

| 缺陷 | 表现 | 改法 |
|---|---|---|
| `not in` 没处理 | `timediff not in ('1-10分钟')` 翻成「**not** 属于」,只翻掉 in 剩个光秃秃的 not —— **语义正好反过来**。模型规则里 `not in` 是常客 | 在 `in` 之前先匹配 `not in` → 「不属于」;顺带支持 `.isin([...]) == False`。已固化回归 |
| 回退标签是整句描述 | registry 里不少描述是「口径,作用说明」两段式,整句当标签塞进条件句会变成读不断的一长串(`pre_first_expose_to_touch_min` → 「…时间差（分钟）,反映用户活跃时长 ≥ 8」) | 回退时切掉逗号后的解释从句,**逗号前一个字不动**(宁可长,不猜) |

**验证**(`tests/test_crowd_filter_zh.py`,新增,26 项;`test_condition_zh.py` 加 2 项):原有键与值逐条未变;整批翻不动时不产生假中文;渲染层抛异常时规则条数与 `sql_filter` 不变;两处翻译同源比对;`not in` 语义回归。API 侧五套回归 31/59/109/166/172 全过,新增 `regress_contract` 22 过。

**仍未动、等决定**:短标签的窗口口径混用(「近1天最深漏斗深度」与「预订页回退次数」同句)、`pre_mkt_channel_cnt` 等 9 个字段缺短标签只能回退长描述、`pre_first_expose_to_touch_min` 的 registry 描述疑似写成了活跃时长(**口径存疑,已上报未改**)。

---

## ★ 2026-08-12 附录人群包：中文主显 + 原条件折叠(fix28)

**动机**:fix24 把中文当作**第四列**加进来。拿用户线上已产出的真实报告(1012006)套上去看,四栏挤在一起,英文 `code` 一行折三次,中文反而更难读 —— **加了一列信息,可读性却更差**。

**改法**:回到三列,条件格里**中文主显、英文原条件收进 `<details>`**,点「原条件 ▾」展开。真实报告上表格高度少了约四成,中文能一行读完。

**三条硬约束**(顺序即优先级):

1. **`filter_conditions` 原文逐字保留在 DOM 里** —— 只是默认不显示。下游圈人复制到的仍是同一份执行口径,`humanize_condition` 依旧只是展示层投影,不回写 state;
2. **翻不动就不折叠** —— `humanize_condition` 翻不出时会**原样回显**输入,这时折叠等于把同一串英文显示两遍还藏了一半。判据是 `译文 != 原文`,不是「译文非空」;
3. **打印/导出 PDF 不能丢原条件** —— `<details>` 收起时内容默认不进 PDF,而附录正是给下游做审计用的。双保险:`beforeprint` 钩子强制展开(主)+ `@media print` 隐藏 summary、`::details-content{content-visibility:visible}` 兜底,`afterprint` 还原用户原本的展开状态。已用 headless 打 PDF 验证:原条件在、`原条件 ▾` 标签不在。

**交互沿用附录现有的「展开 ▾」**(原生 `<details>`,零 JS),报告被外链嵌入宿主页时不引入新行为。

**验证**(`tests/test_condition_zh.py`):表头断言由四列改三列(**有意改动**);新增「原条件仍在 DOM 里」逐条比对、「折叠数 == 人群数」、「翻不动时不折叠且原条件直接可见」、「beforeprint 已挂载」。区块边界的正则由 `.*?</details>` 改为 `.*?</table>` —— 条件格里现在嵌了内层 `<details>`,老写法会停在第一个折叠上(**这是改测试时真踩到的,不是预防性改动**)。

---

## ★ 2026-08-12 附录人群包 → 核心发现回跳(fix27)

**动机**:此前只有单向 —— 核心问题卡的人群 chip 跳到附录锚点。反过来在附录看到一个人群,想知道它是哪条发现推出来的,只能自己往上翻。

**做法**:`_build_segment_backlinks(problems, actions)` 沿 `segment.name → action.target_audiences → action.problem_rank` 反查到核心发现,人群名称后挂一个 `↑ #N` 徽标,hover 显示完整发现标题。

**编号口径必须与 `_chapter1` 的 `problem-{i+1}` 同源** —— 两处都用 problems 列表下标算,不用 `problem_rank` 本身。用 rank 当锚点看着更直觉,但 rank 不保证连续也不保证从 1 起,一旦上游改排序就是**满屏死链**。已固化断言:每个回跳目标都必须是报告里真实存在的 `problem-N`。

**对不上的人群不加链接**,保持纯文本 —— 模型分析产出的人群(如「深漏斗高潜人群·变体2」)本就没有对应的规则发现,给死链比不给链接更糟。真实报告上 6 个人群命中 4 个。

**跳转走 JS `scrollIntoView` 而非裸 `href="#x"`**:报告被外链嵌入后 base URL 变成宿主页,`#x` 会被解析成绝对地址触发整页跳转(见《处置方案_20260803》待办 #3)。处理器只认 `.fp-jump` 类,**现有的正向 chip 与左侧目录一个字没动** —— 那两处要不要一起改仍待定。落点会闪 1.4 秒红色描边,让人看清跳到哪了。

**验证**(`tests/test_backlink.py`,新增,16 项断言全过):编号按列表下标而非 rank;`target_audiences` 里的裸字符串也能对上;对不上的不进映射;结构异常返回空表而非抛异常(**附录少几个链接,好过报告渲不出来**);每个回跳落在真实存在的卡片上;`href` 与 `data-to` 一致;无行动方案时不产生回跳。

**已在用户线上已产出的真实报告上验收通过**(1012006:无主流程低意向人群→#1、自然转化营销多余人群→#2、创单未付待促付人群→#3、多渠道冲突人群→#4)。

---

## ★ 2026-08-12 #5/#21 叠加长周期画像字段,修正"用一天行为下长期结论"(fix26)

**动机**:fix25 查明特征表是**近1天窗口**后,复核发现 A 级 7 条规则在用一天行为下长期性格判断。本次先改风险最高的两条 —— 它们的建议动作都会直接影响投放圈人。

| 规则 | 问题 | 改法 |
|---|---|---|
| **#5 僵尸用户浪费营销** | 「沉默用户」实为**近1天**无主流程行为。一天没动静 ≠ 僵尸,而建议是"减少营销触达",按一天判定会把大量正常用户划进冷却名单 —— **误伤面最大的一条** | 追加 `visit_days <= threshold('visit_days','p25')`。近1天的三个条件负责"当日无动作",近90天访问天数负责"长期低活跃",两半都成立才判沉默 |
| **#21 缺少临门一脚优惠** | 原口径靠「无任何领券记录」倒推价格敏感,**方向本就存疑**(不领券也可能是不需要优惠);叠加近1天窗口后,「今天没领券」更推不出价格不敏感 | 追加 `serialid_bonus >= threshold('serialid_bonus','p75')`。近1天的"今天还没给过优惠"负责判断**时机**,近1年促销订单占比负责判断**价格敏感度** —— 分工清楚,不再靠缺失倒推 |

**画像缺失时整条 skipped,不退化**:两个新字段 `fallback: null`,画像未 join 上则规则 skipped(有断言)。这是有意的 —— **宁可不报,也不用一天行为下长期结论**;skipped 会被 fix22 的问题账本自动记成 `field_missing`,跨 job 累计后自然浮现"画像 join 长期缺失"这个数仓待办。

**⚠️ 这是全库第一次使用非 `optimal` 的分位数 stat**(`p25`/`p75`),正好压中 fix23 修的那个坑:老代码 `ft.get(stat) or ft.get("optimal")` 在 `visit_days` 的 p25 恰为 0 时(近90天访问天数的 25 分位很可能就是 0)会**静默换成 optimal**,阈值从 0 跳到 45,触发人群完全变样。fix23 之前这个 bug 是潜伏的,fix26 之后它会真的踩到 —— 回归用例已固化(`p25=0.0` 时触发数必须等于按 0 算的结果,不得等于按 optimal 算的结果)。

**验证**(`tests/test_rules_fix26.py`,新增,13 项断言全过):分位数 stat 解析正确(用 p25=22 而非 optimal=45,逐行手算比对);两条规则**触发人数确实收紧**(#5 1397→351、#21 260→72);画像缺失时 skipped 且 skip_reason 点名字段;`threshold_used` 记录两个阈值来源;`p25=0.0` 的 fix23 回归;两条规则都带 `data_note` 且描述里同时点明近1天与近90天/近1年口径。

**连带**:`feature_registry.yaml` 给 `visit_days`/`serialid_bonus` 挂上规则引用;`methodology/08` 更新两条建议方向并新增窗口口径提醒(明确其余 A 级规则 #11/#19/#25/#27/#43 **尚未**叠加长周期字段,叙述必须限定在「当日」);`tests/smoke_fix21.py` 的冻结快照移除 #5/#21(**有意改动,非回归**),剩 20 条继续守。

---

## ★ 2026-08-12 特征描述对齐权威建表语句(fix25)

**来源**:业务方提供 `app_dm.marketing_audit_base_feature_activity` 的建表语句(186 个字段的 COMMENT),作为特征中文描述的权威口径。

**⚠️ 最重要的不是改了措辞,是发现了口径错**:权威注释里 **170/183 个字段以「近1天」开头**,而 skill 的描述有 **103 个写的是「历史」或「触达前」** —— 读起来像"全量历史",实际这张表按 date 分区、只覆盖**近 1 天**。这不是文案问题,是**会误导规则解读**的口径错:
- `pre_is_repurchase` 原写「历史是否有多次成单(复购用户)」→ 实为「近1天是否有多次成单」。用"近1天内多次成单"定义的复购人群,比全量历史复购小几个数量级。
- `pre_popup_touch_cnt` 原写「历史弹屏触达次数」→ 实为「近1天弹屏触达次数」。规则 #35「创单前弹屏过多」的阈值一直是按"近1天"算的,但文档让人以为是历史累计。
- 同类还有 `pre_create_order_cnt`(#25 多次创单未付)、`pre_mkt_touch_cnt`、`pre_has_coupon`(#19/#21) 等。
**规则的计算逻辑没有变**(一直就是这张表),变的是描述终于说对了它算的是什么。建议按新口径复核一遍受影响规则的业务含义。

| 变更 | 说明 | 文件 |
|---|---|---|
| **172 条描述对齐 COMMENT** | 逐字段替换为权威注释 | `feature_schema/feature_registry.yaml` |
| **116 行字段说明同步** | Agent 查字段语义读的就是这份 | `references/behavior_fields.md` |
| **7 条短标签改口径** | 报告里露出的短名(如「历史弹屏触达次数」→「近1天弹屏触达次数」),保持短的同时不再误导 | `snippets/report_renderer.py` |
| **6 条按同族推断** | `pre_first_mkt_time` 等 6 个 SQL 未登记,但同族(`pre_first_mkt_channel` 等)权威注释已是「近1天」、同表同窗口,一并对齐;**已在交付清单标注为推断而非权威来源** | 同上 |
| **2 条跳过不改** | `deviceid`/`unionid` 的 COMMENT 就是英文字段名本身,照抄会把现有中文("设备ID"/"微信 Union ID")换成英文 —— 保留原值 | 同上 |
| **否定词按完成体选「未/不」** | 新描述带来的病句:「近1天是否浏览过X」的否定被拼成"不浏览过X"。改为 B 含「过」时用「未」("近1天未浏览过活动目标品类"),否则用「不」("站内多渠道推送品类不一致") | `snippets/report_renderer.py` |

| **F 组 25 条补窗口前缀(fix25.1)** | 建表语句只登记了「…platform/…channel/…model」那一支(注释用「时间/品类/平台」一句话覆盖整族),`pre_first_coupon_time`/`pre_first_coupon_product` 等展开项对不上名字。经业务确认**这些字段线上确实存在**,按同表同窗口给现描述统一加「近1天」前缀。至此 registry 里**全部 pre_* 描述都带窗口前缀**,无一遗漏 | `feature_registry.yaml`、`references/behavior_fields.md` |

**验证**(`tests/test_condition_zh.py` 扩到 47 项断言全过):新增**口径固化断言** —— registry 里任何 `pre_*` 描述再出现「历史」即失败,防止日后改回去;三个代表字段的描述前缀逐字比对;短标签表同步断言。中文筛选条件列的全部用例按新口径重比。fix21～24 全套断言数字不变,`cli doctor` 全绿。

---

## ★ 2026-08-12 人群包新增「中文筛选条件」列(fix24)

**动机**:附录「可落地人群包」的筛选条件是 pandas 布尔表达式(`pre_train_depth<1 and pre_create_order_cnt>=1`),给下游圈人用没问题,但运营看报告时读不懂 —— 这张表恰恰是业务方最需要看懂的一张。

**做法**:表头由三列扩到四列(人群名称 / 筛选条件 / **中文筛选条件** / 建议动作),中文列由 `humanize_condition()` 在渲染时现算。

三条边界写死在实现里:
- **原文一个字都不改**。`filter_conditions` 是下游圈人的执行口径,中文列只是**展示层投影**,不进 state、不进 `crowd_rules.json`(有断言:渲染前后原文逐字相同、state 里不出现 `filter_conditions_zh`)。
- **翻不动就原样保留,绝不猜**。认不出的字段名/片段照抄英文 —— 宁可露一小段英文,也不能把语义猜错。
- **永不崩**。整个函数 try/except 兜底返回空串;条件是垃圾串时报告照常渲染(有断言)。

**翻译覆盖**:字段名→中文标签(复用 `_humanize_feature`,与典型案例/模型解读**同一套口径**,前后叫法一致);二值字段 `==1/==0` → 「已转化」「未成单」「无历史领券行为」(先剥掉标签里的"是否",再按 有/无 或 已/未 选词);`.str.contains('详情')` → 包含「详情」;`.notna()/.isna()` → 不为空/为空;`.isin([...])`、`in (...)` → 属于;运算符 → ≥ ≤ ≠ =;`&`/`and` → 且,`|`/`or` → 或;算术表达式与"字段 vs 字段"比较照常翻(`(A+B) >= 3`、`领券平台 ≠ 主用平台`)。

**顺带查出并修掉的两个真问题**(都是加这一列时才暴露的):
- **`feature_registry.yaml` 里 3 条描述被 YAML 静默截断**。未加引号的 YAML 标量中,空格后的 `#` 会被当作行注释 —— `…，诊断 #12）` 的后半截被吃掉,描述只剩一个**不闭合的左括号**。这三个字段(`ads_no_insite_flag`/`ads_insite_match_flag`/`insite_multi_channel_match_flag`)的中文名于是变成 `站内多渠道推送品类是否一致（1=一致 0=不一致 NULL=站内不足2种渠道，诊断`,原样泄进报告。已给三条描述加引号;并给 `_humanize_feature` 的括号剥离补一层"左括号后没有右括号"的兜底。测试加了全表断言:任何描述的 `（`/`）` 数量不等即失败。
- **「A是否B」型标签被压成光秃秃的 B**。`_binary_phrase` 原来从"是否"处一刀切,`站内多渠道推送品类是否一致 == 0` 渲染成「无一致」—— 运营看不出说的是哪件事。改为保留限定语:`A是否B` → `AB` / `A不B`(→「站内多渠道推送品类不一致」),`A是否有B` → `A有B` / `A无B`(→「触达前无目标品类主流程浏览」),仅当没有限定语时才用「已/未」「有/无」。

**验证**(`tests/test_condition_zh.py`,新增,39 项断言全过):9 种真实条件形态逐条比中;**每条输出都断言无残留英文字段名、无 ASCII 比较符**;空/空白/畸形输入的容错;拿 `examples/output_example.json` 端到端渲染,表头恰为四列、8 个人群各一行四格、中文列 100% 有内容;原文逐字未变;标签与 `_humanize_feature` 一致。fix21/22/23 全套断言数字不变。
