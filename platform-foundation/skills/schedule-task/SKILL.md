...
```bash
bash /home/jovyan/.claude/skills/schedule-task/_shared/capture_env.sh
```
...
```bash
bash /home/jovyan/.claude/skills/schedule-task/_shared/preflight.sh          # 常规
bash /home/jovyan/.claude/skills/schedule-task/_shared/preflight.sh --deep   # 加验环境是否穿透到 Bash 工具
```
...
5. **挂 crontab**（幂等，且自动备份条目）：
   ```bash
   bash /home/jovyan/.claude/skills/schedule-task/_shared/add_task.sh <任务名> "<cron表达式>" [多久没跑算异常/小时]
   ```
...
```
~/.claude/skills/schedule-task/       # skill 目录(核心工具)
├── SKILL.md
├── install.sh
├── _shared/
...
/home/jovyan/schedule_task/             # 任务根目录(只存任务实例)
└── <任务名>/
