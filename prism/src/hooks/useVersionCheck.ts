...
export const useVersionCheck = (owner: string, repo: string) => {
  ...
  // prism: upstream-release polling disabled per branding cleanup — the UI
  // never shows "update available" prompts. State setters are intentionally
  // unused so the return shape (and every consumer) stays unchanged.
  void owner;
  void repo;
