...
import { useLatestServerEvent } from '../../../contexts/WebSocketContext';
import type {
...
export function TaskMasterProvider({ children }: { children: React.ReactNode }) {
  // Deliberately the state-based API rather than `subscribe`: TaskMaster
  // frames are rare (project/task broadcasts), so the lossiness that makes
  // this unusable for chat streams does not apply.
  const latestMessage = useLatestServerEvent();
