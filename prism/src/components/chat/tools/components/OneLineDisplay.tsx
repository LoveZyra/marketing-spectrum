...
export const OneLineDisplay: React.FC<OneLineDisplayProps> = ({
  ...
  const renderCopyButton = () => (
    <button
      ...
      className="ml-1 flex-shrink-0 text-muted-foreground opacity-0 transition-colors hover:text-foreground group-hover:opacity-100"
      ...
      <div className="group my-1">
        <div className="flex items-start gap-2">
          ...
          <div className="flex min-w-0 flex-1 items-start gap-2">
            <div className="min-w-0 flex-1 rounded bg-muted px-2.5 py-1 ">
              <code className={`font-mono text-xs text-foreground dark:text-primary ${wrapText ? 'whitespace-pre-wrap break-all' : 'block truncate'}`}>
                <span className="select-none text-foreground dark:text-primary">$ </span>{value}
              </code>
            </div>
  ...
  if (action === 'open-file') {
    ...
    return (
      <div className={`group flex items-center gap-1.5 border-l-2 ${colorScheme.border} my-0.5 py-0.5 pl-3`}>
        ...
        <button
          onClick={handleAction}
          className="truncate font-mono text-xs text-foreground transition-colors hover:text-primary/80 hover:underline dark:text-primary"
          title={value}
        >
          {displayName}
        </button>
      ...
      <div className={`group flex items-center gap-1.5 border-l-2 ${colorScheme.border} my-0.5 py-0.5 pl-3`}>
        ...
        {toolResult && (
          <a
            ...
            className="flex flex-shrink-0 items-center gap-0.5 text-[11px] text-foreground transition-colors hover:text-primary/80 dark:text-primary"
          >
            <svg className="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
