        ...
        {files.map((file, index) => {
          ...
          return (
            <span key={index} className="inline-flex items-center">
              <button
                onClick={handleClick}
                className="font-mono text-[11px] text-foreground transition-colors hover:text-primary hover:underline dark:text-primary"
                title={filePath}
              >
                {fileName}
              </button>
              {index < files.length - 1 && (
                <span className="ml-1 text-[10px] text-foreground">,</span>
              )}
            </span>
          );
        })}
