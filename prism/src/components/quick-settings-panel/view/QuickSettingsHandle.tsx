...
export default function QuickSettingsHandle({
  ...
  return (
    <button
      ...
      className={`fixed ${placementClass} z-50 ${transitionClass} border bg-background ${borderClass} prism-modal-shadow rounded-l-md p-2 transition-colors hover:bg-muted ${cursorClass} touch-none`}
      style={{
        ...style,
        touchAction: 'none',
    ...
    >
      {isDragging ? (
        ...
        <ChevronRight className="h-5 w-5 text-body" />
      ) : (
        <ChevronLeft className="h-5 w-5 text-body" />
      )}
    </button>
  );
}
