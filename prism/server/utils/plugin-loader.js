...
// Plugins live under the resolved data dir so they follow PRISM_DATA_DIR and
// the one-time ~/.cloudcli -> ~/.prism migration. These used to point at
// ~/.claude-code-ui — the upstream project's folder, which this fork has not
// written to since it was renamed, so installed plugins were invisible.
//
// Resolved per call rather than frozen into a module constant: getDataDir()
// reads PRISM_DATA_DIR live, and this module can be imported before the CLI
// finishes loading .env.
const pluginsDirPath = () => path.join(getDataDir(), 'plugins');
const pluginsConfigPath = () => path.join(getDataDir(), 'plugins.json');
...
export function savePluginsConfig(config) {
  ...
  fs.writeFileSync(pluginsConfigPath(), JSON.stringify(config, null, 2), { mode: 0o600 });
}
