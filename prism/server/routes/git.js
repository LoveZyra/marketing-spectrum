...
// Input validation and argv construction live in ./git-args.js so they can be
// unit tested; see the threat-model comment at the top of that file. The rule
// of thumb for this router: never hand-write an argument array containing a
// user-supplied ref — call the matching builder, which validates and places the
// `--` separator correctly for that specific subcommand.
...
router.post('/checkout', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', checkoutBranchArgs(branch), { cwd: projectPath });
...
router.post('/create-branch', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', createBranchArgs(branch), { cwd: projectPath });
...
router.post('/delete-branch', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', deleteBranchArgs(branch), { cwd: projectPath });
    res.json({ success: true, output: stdout });
  } catch (error) {
...
router.get('/commit-diff', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', showCommitArgs(commit), { cwd: projectPath });
...
router.post('/generate-commit-message', async (req, res) => {
  ...
  // Validate provider (Claude is the only provider)
  if (provider !== 'claude') {
...
Generate the commit message:`;
  ...
  try {
    ...
    const writer = {
      send: (data) => {
        try {
          ...
          // Handle message formats from the Claude SDK
    ...
    // Call the agent (Claude is the only provider)
    if (provider === 'claude') {
...
router.post('/fetch', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', fetchArgs(remoteName), { cwd: projectPath });
...
router.post('/pull', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', pullArgs(remoteName, remoteBranch), { cwd: projectPath });
...
router.post('/push', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', pushArgs(remoteName, remoteBranch), { cwd: projectPath });
...
router.post('/publish', async (req, res) => {
  ...
  try {
    ...
    const { stdout } = await spawnAsync('git', pushSetUpstreamArgs(remoteName, branch), { cwd: projectPath });
