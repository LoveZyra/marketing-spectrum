...
// Claude Code CLI is the only supported provider. readModelProvider() still
// normalizes through this list so a stale `provider` field from an older client
// or a persisted session falls back to "claude" instead of being echoed back.
...
const builtInCommands = [
  {
    name: "/help",
    description: "查看 Claude Code 帮助文档",
    namespace: "builtin",
    metadata: { type: "builtin" },
  },
  {
    name: "/models",
    description: "查看当前供应商可用的模型",
    namespace: "builtin",
    metadata: { type: "builtin" },
  },
  {
  ...
  {
    name: "/memory",
    description: "打开并编辑 CLAUDE.md 记忆文件",
    namespace: "builtin",
    metadata: { type: "builtin" },
  },
  {
...
/**
 * Prism: CLI-native commands surfaced in the menu for discoverability.
 * These are NOT executed by the web UI — the composer sends them through
 * as-is and the Claude Code runtime handles them natively. A live session
 * can extend this list dynamically via GET /api/claude/slash-commands.
 */
const cliPassthroughCommands = [
  { name: "/loop", description: "Agent Loop：目标→执行→测试→修复自动循环（--rounds N --test \"命令\"）" },
  { name: "/compact", description: "压缩当前对话上下文（原生，Session ID 不变）" },
  { name: "/context", description: "查看当前上下文占用明细" },
  { name: "/init", description: "分析项目并生成 CLAUDE.md" },
  { name: "/review", description: "审查代码改动或 PR" },
  { name: "/security-review", description: "对当前改动做安全审查" },
  { name: "/pr-comments", description: "查看当前分支 PR 的评论" },
  { name: "/release-notes", description: "查看 Claude Code 版本变更" },
  { name: "/todos", description: "列出当前任务清单" },
  { name: "/rewind", description: "回退对话/文件状态" },
].map((command) => ({
  ...command,
  namespace: "cli",
  metadata: { type: "cli" },
...
router.post("/list", async (req, res) => {
  try {
    ...
    res.json({
      builtIn: [...builtInCommands, ...cliPassthroughCommands],
