...
// The CLOUDCLI_* names are the pre-rename spelling. They are still read as a
// fallback because an MCP entry registered before the upgrade has the old names
// baked into its stored `env` block, and that entry keeps launching this script
// until the user re-enables browser-use (which re-registers with PRISM_*).
const apiUrl = (
...
const API_TIMEOUT_MS = Number.parseInt(
  process.env.PRISM_BROWSER_USE_API_TIMEOUT_MS
...
async function callBrowserUseApi(toolName: string, input: Record<string, unknown>) {
  if (!apiToken) {
    throw new Error('PRISM_BROWSER_USE_MCP_TOKEN is not configured.');
  }
...
async function handleMessage(message: JsonRpcRequest) {
  if (message.method === 'initialize') {
    return {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: 'prism-browser', version: '1.0.0' },
    };
  }
