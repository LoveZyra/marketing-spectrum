...
import { LEGACY_MANAGED_MCP_SERVER_NAMES, MANAGED_MCP_SERVER_NAME } from '../../../shared/managedMcpServers.js';

import { appConfigDb } from '@/modules/database/index.js';
import { providerMcpService } from '@/modules/providers/index.js';
import { getDataDir, getModuleDir } from '@/utils/runtime-paths.js';

const require = createRequire(import.meta.url);
const __dirname = getModuleDir(import.meta.url);
const IS_PLATFORM = process.env.VITE_IS_PLATFORM === 'true';
const MAX_SESSIONS_PER_OWNER = Number.parseInt(process.env.PRISM_BROWSER_USE_MAX_SESSIONS_PER_OWNER || '3', 10);
const SESSION_TTL_MS = Number.parseInt(process.env.PRISM_BROWSER_USE_SESSION_TTL_MS || String(30 * 60 * 1000), 10);
...
/**
 * Moves a session into the `stopped` state.
 *
 * Exported so the release invariant is unit-testable without a real browser:
 * a stopped session MUST drop `screenshotDataUrl`. That field holds a full
 * base64 JPEG (hundreds of KB), and only deleteSession() ever removes an entry
 * from the sessions Map — so a stop path that forgets to null it pins that
 * image for the lifetime of the process. There are three stop paths (idle
 * expiry, explicit stop, server shutdown) and each one previously repeated the
 * field assignments by hand, which is how the leak got in.
 */
export function markSessionStopped(
  session: BrowserUseSession,
  options: { lastAction: string; message: string; at?: number },
): BrowserUseSession {
  session.status = 'stopped';
  session.screenshotDataUrl = null;
  session.updatedAt = new Date(options.at ?? Date.now()).toISOString();
  session.lastAction = options.lastAction;
...
/**
 * Persistent Chromium profiles live under the resolved data dir rather than a
 * hardcoded home-relative path, so they follow PRISM_DATA_DIR and the one-time
 * ~/.cloudcli -> ~/.prism migration instead of pointing at a folder that the
 * migration already renamed away.
 */
const PROFILE_ROOT = path.join(getDataDir(), 'browser-use', 'profiles');
const MCP_SERVER_NAME = MANAGED_MCP_SERVER_NAME;
const LEGACY_MCP_SERVER_NAMES = LEGACY_MANAGED_MCP_SERVER_NAMES;
...
function getMcpCommand(): { command: string; args: string[] } {
  ...
  // Fallback for installs where the bundled script is missing: invoke the CLI
  // by the name package.json actually declares in "bin". This used to say
  // 'cloudcli', which is not a binary this package installs.
  return {
    command: 'prism',
...
const INSTALL_COMMAND_TIMEOUT_MS = Number.parseInt(
  process.env.PRISM_BROWSER_USE_INSTALL_TIMEOUT_MS || String(10 * 60 * 1000),
  10,
);
...
function formatInstallError(error: unknown): string {
  ...
  if (message.includes('sudo') && message.includes('password')) {
    return 'Installing Chromium system dependencies requires administrator privileges. Run `npx playwright install-deps chromium` on the machine where Prism runs, then try again.';
  }
...
export const browserUseService = {
  ...
  async registerAgentMcp() {
    ...
    const results = await providerMcpService.addMcpServerToAllProviders({
      ...
      args,
      env: {
        PRISM_BROWSER_USE_MCP_TOKEN: getOrCreateMcpToken(),
        PRISM_BROWSER_USE_API_URL: getMcpApiUrl(),
      },
    });
  ...
  async stopSession(sessionId: string) {
    ...
    markSessionStopped(session, {
      lastAction: 'stop',
      message: 'Browser session stopped. Create a new session to continue browsing.',
    });
  ...
  async stopAllSessions() {
    await Promise.all([...sessions.keys()].map(async (sessionId) => {
      ...
      if (session) {
        markSessionStopped(session, {
          lastAction: 'shutdown',
          message: 'Browser session stopped during server shutdown.',
        });
      }
    }));
  },
