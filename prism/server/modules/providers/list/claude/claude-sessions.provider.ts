...
/**
 * Identifies the transcript backing a session, for cache validity.
 *
 * mtime alone is not enough: filesystems report it at coarse granularity, so
 * two appends inside the same tick would share an mtime and the second read
 * would be served a stale, truncated history. Size disambiguates, and a
 * transcript is append-only, so the pair is sufficient.
 *
 * Returns null when the session has no indexed transcript or the file cannot be
 * stat()ed, which makes the caller skip the cache entirely rather than key an
 * entry on something it cannot revalidate.
 */
async function statTranscript(
  sessionId: string,
): Promise<{ fingerprint: string; bytes: number } | null> {
  const jsonLPath = sessionsDb.getSessionById(sessionId)?.jsonl_path;
  if (!jsonLPath) {
    return null;
  }
  ...
  try {
    const stats = await fsp.stat(jsonLPath);
    return { fingerprint: `${stats.mtimeMs}:${stats.size}`, bytes: stats.size };
  } catch {
    return null;
  }
}
...
/**
 * Slices a page out of a fully-normalized history.
 *
 * Split out of fetchHistory so the cached and uncached paths cannot drift. They
 * would otherwise be two copies of the same clamping, and a cached page that
 * disagreed with a freshly-read one about `hasMore` would make the frontend
 * either request older messages forever or silently stop loading them.
 */
export function buildHistoryPage(
  ...
  const { page, hasMore } = sliceTailPage(history.messages, normalizedLimit, normalizedOffset);
...
export class ClaudeSessionsProvider implements IProviderSessions {
  ...
  async fetchHistory(
    ...
    // Keyed on both ids because one app session can be replayed against a
    // different provider transcript after a resume.
    const cacheKey = `${sessionId}::${providerSessionId}`;
    const transcript = await statTranscript(sessionId);
    const cached = transcript ? historyCache.get(cacheKey, transcript.fingerprint) : null;
    if (cached) {
      return buildHistoryPage(cached, limit, offset);
    }
    ...
    return buildHistoryPage({ messages: normalized, total }, limit, offset);
  }
}
