...
/** Same key resolution as credentials.ts — see the note there. */
function decryptCredentialValue(value: string): string {
  return decrypt(
    value,
    getEncryptionKey({
      loadPersistedKey: () => appConfigDb.get('credential_encryption_key'),
      savePersistedKey: (hex: string) =>
        appConfigDb.set('credential_encryption_key', hex),
    })
  );
}
