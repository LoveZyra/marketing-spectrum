...
/**
 * The 'opencode' provider tag here is deliberate, not leftover. Prism now ships
 * Claude only, but a database carried over from an older install still holds
 * rows written by the removed providers. The DB layer must keep resolving them
 * rather than treating an unknown provider string as a corrupt row.
 */
