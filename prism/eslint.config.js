...
export default tseslint.config(
  ...
  },
  {
    ...
    rules: {
      ...
      // --- Secure-context APIs ---
      // navigator.clipboard is undefined outside a secure context, and Prism is
      // routinely served over plain HTTP on a LAN address. Reading .writeText
      // off it there throws synchronously — inside a click handler that takes
      // the interaction down with it. src/utils/clipboard.ts feature-detects
      // and falls back to execCommand; everything else goes through it.
      // Matches the UNGUARDED form only ([optional=false]): `navigator.clipboard
      // ?.writeText` inside a feature detection is the correct spelling and
      // stays legal, and reads are left alone — they are guarded inline at
      // their two call sites and have no shared helper.
      "no-restricted-syntax": [
        "error",
        {
          selector:
            "MemberExpression[optional=false][object.object.name='navigator'][object.property.name='clipboard'][property.name='writeText']",
          message:
            "navigator.clipboard is undefined over plain HTTP — use copyTextToClipboard from src/utils/clipboard.ts.",
        },
      ],
  ...
  {
    // The one module allowed to touch the raw API — it is the feature detection.
      ...
      "boundaries/elements": [
        ...
        },
        {
          ...
          pattern: [
            ...
            "server/shared/download-tickets.js", // 下载直传的短命票据:一次普通导航设不了 Authorization 头,凭据只能进 URL
            "server/shared/download-headers.js", // 下载响应头:Content-Length(进度条的硬前提)+ RFC 5987 文件名
            "server/shared/image-downscale.ts", // 发给模型前在内存里缩图(长边 1568 / ~1MB),磁盘原图不动;只依赖 sharp 与 logger
