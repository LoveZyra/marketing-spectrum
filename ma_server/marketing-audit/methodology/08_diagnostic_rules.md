...
| 4 人群质量过低 | easy | ① 投放前接入风控+消费价值标签，排除风险用户/近1年负营收/极短停留的低质量人群；② 先核查圈选人群质量与活动-人群匹配度，再决定是否投放 |
| 5 僵尸用户浪费营销 | easy | ① **分层策略**：近30天注册或当天新客→首触欢迎类（内容/榜单/红包模块），注册>30天→按历史出行推同类商品 或 停止营销，公共首页有搜索/点击→按品类定向；② 优先触达近期有主流程行为的高价值用户 |
...
| 7 营销时机滞后 | easy | ① 检查实时场景标签配置是否正确；② 确保营销触发在用户关键行为之前或同时，不滞后 |
| 44 伪实时场景配置 | easy | ① 排查实时场景（`is_today`）是否误配离线节点（`scene_has_offline_node`），改为即时发送；② 校验先知场景标签实时性，避免人群量级缺失与推送时段错配 |

### 内容匹配（Rule 11）

**关注点**：`pre_mkt_product_browse_match=0` 的占比及 CVR

> ✅ **渠道适用性**：Rule 11 已覆盖 `activity`/`push`/`popup`/`sms`/`outbound`/`insite_msg` 渠道。
> 品类匹配（`pre_mkt_product_browse_match`）是这些主动营销渠道通用的内容匹配信号；对**活动渠道**
> 同样适用，常是活动类活动转化最强的可控因素（品类不匹配人群 CVR 通常仅为匹配人群的 1/5）。

| Rule | execution_difficulty | 建议方向（必须覆盖其一） |
|---|---|---|
| 11 跨品类推送错配 | medium | ① 根据用户实时浏览行为动态调整推送品类；② 建立兴趣标签实时更新机制，与用户当前浏览品类保持一致 |
| 43 推送零浏览品类 | medium | ① 排除"目标品类主流程零浏览"人群或改投其真实浏览品类；② 对零浏览品类的推送加种草/低价钩子前置培育 |

> Rule 43 是 Rule 11 的"硬"版本：11 看"目标品类 ≠ 兴趣最深品类"，43 看"目标品类**主流程浏览次数=0**"（用户从没进过该品类的流程页）。两者同属内容匹配主题组（覆盖检查合并，避免重复上报），取触发体量/效应量更强者作主 finding。
...
### 关键打断（Rules 15, 16, 17, 18, 39）

**关注点**：触达前 `pre_last_mainflow_detail` 包含"详情"/"填写"的用户 CVR

> ⚠️ **渠道适用性约束**：Rule 16（弹屏打断支付）、Rule 39（弹屏打扰填写）专为 `popup`/`弹屏` 渠道设计。
> 若 `target_channels` **不含** `popup`/`弹屏`，将 Rule 16、39 标记为 `not_applicable`，不生成 finding。

| Rule | execution_difficulty | 建议方向（必须覆盖其一） |
|---|---|---|
| 15 详情页营销打断 | medium | ① 设置详情页营销静默期；② 用户离开详情页 5 分钟后再触发营销 |
| 16 弹屏打断支付 ⚠️ popup渠道专属 | medium | ① 创单后 15 分钟内禁止弹屏；② 完成支付后再恢复营销触达 |
| 17 填写页营销打断 | medium | ① 用户进入填写页后暂停所有营销触达，直到订单完成或超时；② 填写页设置硬性静默期 |
| 18 营销干扰支付 | medium | ① 创单未付用户优先走短信促付；② 避免其他营销内容在支付决策窗口内打断 |
...
| Rule | execution_difficulty | 建议方向（必须覆盖其一） |
|---|---|---|
| 19 自然转化营销多余 | medium | ① 将满足多信号（自己来+不找券+不点营销+路径直达）的"自然转化型"用户标记，在营销前过滤、通过先知人群包排除；② 接入价敏人群算法，仅对优惠敏感用户补贴 |
| 20 过度营销浪费 | easy | ① 设置单用户单日触达上限（= CVR 拐点 N 次）；② 无响应用户进入 24h 冷却名单 |
| 21 缺少临门一脚优惠 | medium | ① 创单未付用户及时发送专属优惠券或红包；② 设置创单未付自动发券流程；③ 优化优惠门槛，提高核销率 |
| 41 营销触发下单未成单 | medium | ① 创单未付用户及时发送促付提醒或优惠券；② 设置创单未付自动催付流程 |

### 流程体验（Rules 23, 24, 25, 27）

**关注点**：比价犹豫 + 漏斗倒退次数阈值 + 遗单习惯

| Rule | execution_difficulty | 建议方向（必须覆盖其一） |
|---|---|---|
| 23 跨品类比价未决策 | hard | ① 识别比价用户提供跨品类对比工具（中长期）；② 针对比价用户推送限时优惠促进决策 |
| 24 漏斗严重倒退 | medium | ① 优化详情页价格和优惠信息展示；② 提供在线客服或智能助手解答疑问 |
...
**关注点**：历史成单次数（`pre_complete_order_cnt`）高于 CVR 拐点的高频购买人群占比

| Rule | execution_difficulty | 建议方向（必须覆盖其一） |
|---|---|---|
| 40 高频下单 | hard | ① 识别高频用户进行差异化专项运营；② 提供会员专属权益提升复购黏性（中长期） |
...
| 关键打断 | 15, 17 | 合并为"高意图阶段营销打断"一条 finding |
| 遗单召回 | 16, 18, 21, 25 | 合并为"遗单用户策略"一条 finding |
