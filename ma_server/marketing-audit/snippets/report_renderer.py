...
class ReportRenderer:
    ...
    def _humanize_feature(name: str) -> str:
        mapping = {
            ...
            "pre_popup_touch_cnt": "历史弹屏触达次数", "pre_push_touch_cnt": "历史Push触达次数",
            ...
            "pre_create_order_cnt": "历史创单次数", "pre_complete_order_cnt": "历史成单次数",
            ...
            "pre_back_to_list_cnt": "详情返列表次数", "pre_over_mkt_flag": "历史过度触达",
        ...
        if zh:
            return re.sub(r"[（(][^（()）]*[)）]\s*$", "", zh).strip() or zh
        return name
...
def render_fp(rr) -> str:
    ...
    head = (f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8">'
            ...
            f'<script>{_SCROLLSPY}</script></head>')
...
def _appendix(rr, segments) -> str:
    ...
    for k, s in enumerate(segments):
        ...
        s_rows += (f'<tr id="{anchor}" style="scroll-margin-top:16px;border-bottom:{bb}">'
                   f'<td style="padding:10px;vertical-align:top;font-weight:600;white-space:nowrap">{_e(s.get("name",""))}</td>'
                   f'<td style="padding:10px;vertical-align:top"><code style="font-family:var(--mono);font-size:11px;'
                   f'background:var(--chipbg);padding:1px 5px;border-radius:2px;color:var(--chiptx);word-break:break-all">{_e(s.get("filter_conditions",""))}</code></td>'
    ...
    return (
        ...
        + detail_block(f"可落地人群包（{len(segments)} 个）", ["人群名称", "筛选条件", "建议动作"], s_rows, open_=True)
        + cav_block + '</section>'
    )
