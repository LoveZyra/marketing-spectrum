...
| `activity_touch_cnt` | 当日触达该用户的次数（单日数据） |
| `activity_click_cnt` | 触达中用户点击次数（action='点击'） |
| `is_activity_clicked` | 是否有任意点击（0/1） |
| `activity_over_touch_flag` | 当日触达≥5次，疑似过度触达（0/1） |
...
| 特征名 | 含义 |
|---|---|
| `is_converted` | 触达当日、`product_name` 匹配的创单（0/1） |
| `is_paid` | 触达当日、`product_name` 匹配的成单（0/1，区分创单未付，**目标变量**） |
...
| 特征名 | 含义 |
|---|---|
| `pre_first_expose_to_touch_min` | 触达前首条行为到最后触达时刻的时间差（分钟），反映用户活跃时长 |
| `pre_last_mainflow_to_touch_min` | 最近一次主流程行为到最后触达时刻的时间差，意向活跃度指示 |
| `pre_last_mkt_to_touch_min` | 上一次历史营销到本次触达的时间差，反映营销频率 |
| `pre_last_order_to_touch_min` | 最近历史成单到本次触达的时间差，反映距上次下单时长 |
...
| 特征名 | 含义 |
|---|---|
| `pre_max_funnel_depth` | 历史最深漏斗阶段（1=首页 2=列表 3=详情 4=填写 5=支付，跨品类） |
| `pre_target_product_funnel_depth` | 活动目标品类的历史最深漏斗阶段 |
| `pre_funnel_pages_cnt` | 访问过的不同主流程页面数（去重） |
| `pre_mainflow_event_cnt` | 主流程行为总次数 |
| `pre_reached_homepage` | 是否曾到达项目首页（0/1） |
| `pre_reached_list` | 是否曾到达列表页（0/1） |
| `pre_reached_detail` | 是否曾到达详情页（0/1） |
| `pre_reached_booking` | 是否曾到达填写页（0/1） |
| `pre_reached_payment` | 是否曾到达支付页（0/1） |
| `pre_back_to_list_cnt` | 从详情页返回列表页的历史次数（犹豫/比价信号） |
| `pre_back_to_booking_cnt` | 从填写页返回详情页的历史次数 |
| `pre_skip_detail_flag` | 历史上是否有跳过详情页直接进入填写页（熟客快速购买信号，0/1） |
| `pre_total_touch_cnt` | 触达前总行为次数（含全类型事件） |
| `pre_browse_target_product` | 触达前是否有目标品类主流程浏览（0/1） |
| `pre_target_product_visit_cnt` | 触达前目标品类主流程访问次数 |
...
| 特征名 | 含义 |
|---|---|
| `pre_mkt_touch_cnt` | 触达前历史营销总触达次数（含本活动中间触达） |
| `pre_mkt_channel_cnt` | 触达前被几种不同营销渠道触达 |
| `pre_touched_popup/push/sms/ads/insite_msg/activity` | 各渠道是否有历史触达（0/1 标志） |
| `pre_popup_touch_cnt` | 历史弹屏触达次数（用于"创单前弹屏过多"阈值判断） |
| `pre_push_touch_cnt` | 历史 Push 触达次数 |
| `pre_sms_touch_cnt` | 历史短信触达次数 |
| `pre_ads_touch_cnt` | 历史广告触达次数 |
| `pre_insite_msg_touch_cnt` | 历史站内信触达次数 |
| `pre_activity_touch_cnt` | 历史活动营销触达次数 |
| `pre_mkt_click_cnt` | 历史营销点击总次数 |
| `pre_has_mkt_click` | 是否有任意历史营销点击（0/1） |
| `pre_popup_click_cnt` | 历史弹屏点击次数 |
| `pre_push_click_cnt` | 历史Push点击次数 |
| `pre_popup_click_rate` | 历史弹屏点击率（NULL=无弹屏触达） |
| `pre_push_click_rate` | 历史Push点击率（NULL=无Push触达） |
| `pre_mkt_fatigue_cnt` | 历史营销后5分钟内离开的次数（营销疲劳信号） |
| `pre_mkt_direct_exit_cnt` | 历史营销后直接退出（5分钟内无行为）的次数 |
| `pre_popup_reject_cnt` | 历史弹屏10秒内即离开的次数（强拒绝信号） |
| `pre_funnel_regression_after_mkt` | 历史营销后漏斗倒退次数（营销干扰购买流程） |
| `pre_mkt_trigger_mainflow_cnt` | 历史营销5分钟内触发主流程的次数（营销有效导流） |
| `pre_over_mkt_flag` | 触达前历史营销≥5次（历史过度触达标记，0/1） |
| `pre_min_mkt_response_sec` | 历史最短营销响应时间（秒）|
| `pre_unique_activity_cnt` | 触达前被触达的不同历史活动数 |
| `pre_mkt_touched_target_product` | 历史上是否有推过同一 product_name 的营销（0/1） |
...
| `pre_product_category_cnt` | 历史浏览过的不同品类数 |
| `pre_is_cross_category` | 是否有跨品类浏览（0/1） |
| `pre_browse_target_product` | 历史是否浏览过活动目标品类（0/1） |
| `pre_target_product_depth` | 活动目标品类的历史最深漏斗阶段 |
| `pre_target_product_visit_cnt` | 活动目标品类的历史访问次数 |
| `pre_top_interest_product` | 历史浏览最深的品类（酒店/机票/火车票/景区/用车/汽车票/无浏览） |
| `pre_mkt_product_browse_match` | 活动目标品类是否与用户历史兴趣最深品类一致（0/1） |
| `pre_has_search` | 历史是否有搜索行为（0/1） |
| `pre_search_cnt` | 历史搜索行为次数 |
| `pre_search_hotel/flight/train/scenic` | 是否搜索过各品类入口（0/1） |
| `pre_search_target_product` | 是否搜索过活动目标品类（0/1） |
...
| 特征名 | 含义 |
|---|---|
| `pre_coupon_collect_cnt` | 历史红包/优惠领取总次数 |
| `pre_has_coupon` | 是否有历史领券行为（0/1） |
| `pre_unique_coupon_cnt` | 历史领取的不同红包品类数 |
| `pre_has_blackwhale` | 是否领取过黑鲸相关优惠（高价值会员信号，0/1） |
| `pre_rp_hotel/flight/train/scenic/car/bus/vacation` | 是否领取过各品类红包（0/1） |
| `pre_rp_payment` | 是否领取过支付优惠券（0/1） |
| `pre_rp_blackwhale_card` | 是否领取过黑鲸卡优惠（0/1） |
| `pre_rp_target_product` | 是否领取过活动目标品类的红包（先领券再被触达，价格驱动信号，0/1） |
| `pre_unique_coupon_product_cnt` | 历史领取的不同红包 product_name 数量 |
...
| 特征名 | 含义 |
|---|---|
| `pre_total_event_cnt` | 触达前总行为次数 |
| `pre_first_event_time` | 触达前首条行为时间 |
| `pre_last_event_time` | 触达前末条行为时间 |
| `pre_active_span_min` | 首末行为时间跨度（分钟） |
| `pre_first_active_hour` | 首条行为的小时（0-23） |
| `pre_first_active_period` | 首条行为时段（上午/下午/晚上/深夜） |
| `pre_user_active_period` | 历史行为量最多的时段（用于时机匹配诊断） |
| `pre_model_diversity` | 历史行为涉及的不同 modelname 数量 |
| `pre_unique_touchpoints` | 历史不同行为组合数（营销/主流程/公共页面范围内） |
| `pre_app/wechat/yilong_event_cnt` | 各平台历史行为次数 |
| `pre_is_cross_platform` | 是否跨平台使用（0/1） |
| `pre_primary_platform` | 行为最多的平台（同程APP/微信/艺龙APP） |
| `pre_morning/afternoon/evening/night_cnt` | 各时段历史行为次数 |
| `pre_events_per_hour` | 历史每小时平均行为密度 |
| `pre_homepage_event_cnt` | 历史大首页行为次数 |
| `pre_homepage_module_cnt` | 历史曝光过的不同首页模块数 |
| `pre_banner_exposed` | 是否曝光过 Banner（0/1） |
| `pre_new_user_zone_exposed` | 是否曝光过新人专区（新客信号，0/1） |
| `pre_big_promo_exposed` | 是否曝光过大促/S级活动（0/1） |
| `pre_kongfu_area_exposed` | 是否曝光过金刚区（0/1） |
| `pre_waterfall_exposed` | 是否曝光过瀑布流（0/1） |
| `pre_tile_area_exposed` | 是否曝光过瓷片区（0/1） |
| `pre_pending_pay_viewed` | 是否查看过待支付卡片（遗单召回信号，0/1） |
| `pre_pending_trip_viewed` | 是否查看过待出行卡片（老客复购信号，0/1） |
| `pre_ai_entry_exposed` | 是否触发过顶部 AI 入口（0/1） |
| `pre_add_to_desktop_exposed` | 是否曝光过加桌入口（0/1） |
| `pre_viewed_member_assets` | 是否查看过会员资产/权益入口（0/1） |
| `pre_black_whale_interest` | 是否有黑鲸相关行为（0/1） |
| `pre_checkin_triggered` | 是否触发过签到（0/1） |
| `pre_activity_nav_viewed` | 是否访问过活动导航（0/1） |
| `pre_highlight_activity_viewed` | 是否访问过精彩活动（0/1） |
| `pre_is_dormant_user` | 触达前是否完全无历史行为（沉默用户被唤醒，0/1） |
...
| `pre_is_marketing_first` | 触达前首个行为是否为营销（被动流量进入，0/1） |
| `pre_is_marketing_last` | 触达前末个行为是否为营销（0/1） |
...
| `pre_search_match_target` | 末次搜索品类是否与活动目标品类一致（0/1） |

#### 9f. 行为路径序列

| 特征名 | 含义 |
|---|---|
| `pre_path_model_seq` | 按时间顺序拼接的 modelname 序列，如 `营销->公共页面->主流程` |
| `pre_path_detail_seq` | 按时间顺序拼接的 detailname 序列 |
| `pre_path_major_seq` | 按时间顺序拼接的 majorname 序列（全量事件） |
| `pre_path_product_seq` | 主流程品类序列（标准化 product_category，仅主流程行） |
...
| 特征名 | 含义 |
|---|---|
| `pre_create_order_cnt` | 历史创单次数 |
| `pre_complete_order_cnt` | 历史成单次数 |
| `pre_has_create_order` | 是否有历史创单（0/1） |
| `pre_has_complete_order` | 是否有历史成单（0/1） |
| `pre_complete_product_cnt` | 历史成单涉及的品类数（去重） |
| `pre_has_target_product_order` | 历史是否有活动目标品类的成单（0/1） |
| `pre_has_target_product_create` | 历史是否有活动目标品类的创单（含遗单，0/1） |
| `pre_is_repurchase` | 历史是否有多次成单（复购用户，0/1） |
| `pre_is_target_product_repurchase` | 历史是否有活动目标品类的复购（0/1） |
| `pre_last_order_product` | 最近一次历史成单的品类 |
| `pre_last_order_time` | 最近一次历史成单时间 |
| `pre_create_not_complete` | 历史是否有创单未成单（遗单用户，0/1） |
| `pre_last_order_to_touch_min` | 最近历史成单到本次触达的时间差（分钟） |
...
| 特征名 | 含义 |
|---|---|
| `has_ads_touch` | 当日是否有站外广告（ads）触达（0/1） |
| `ads_product_name` | 站外广告推送的品类（activity_product_name） |
| `has_insite_touch` | 广告触达之后，是否有任意站内渠道跟进（0/1） |
| `first_insite_product_name` | 广告之后首个站内营销活动的品类 |
| `ads_no_insite_flag` | 有站外广告但广告后无任何站内承接（0/1，诊断 #14） |
| `ads_insite_match_flag` | 站外广告与站内首个营销品类是否一致（1=匹配 0=不匹配 NULL=不适用，诊断 #13） |
| `insite_multi_channel_match_flag` | 站内多渠道推送品类是否一致（1=一致 0=不一致 NULL=站内不足2种渠道，诊断 #12） |
| `insite_channel_cnt` | 当日站内营销渠道种数（辅助判断多渠道冲突程度） |
| `insite_product_cnt` | 当日站内营销涉及的不同品类数 |
