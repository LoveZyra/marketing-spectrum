...
# 路径从脚本自身位置推导(<root>/<任务名>/tasks/run.sh),不写死 —— 这样整棵树
# 换个位置、或者在别的机器上演练,都不用改脚本。
_SELF="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="$(dirname "$(dirname "$_SELF")")"
BASE="$ROOT/$TASK_NAME"
LOG_DIR="$BASE/logs"
TASK_DIR="$BASE/tasks"
...
if ! . "$ROOT/_shared/claude_env.sh"; then
...
if [ "${REQUIRE_BIGDATA:-0}" = "1" ]; then
    ...
    if [ -n "$BD_MISS" ]; then
        ...
        echo "    bash $ROOT/_shared/capture_env.sh"
        echo "    bash $ROOT/_shared/preflight.sh"
